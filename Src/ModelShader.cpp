...
bool ModelShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;
	...
	error = wcscpy_s(vsFilename, 128, L"./Shaders/model.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFilename, 128, L"./Shaders/model.ps");
	if (error != 0)
	{
		return false;
	}
...
bool ModelShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, Matrix& matrix, ID3D11ShaderResourceView* texture)
{
	bool result;
...
bool ModelShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	HRESULT result = D3DCompileFromFile(vsFilename, NULL, NULL, "ModelVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
void ModelShader::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool ModelShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, Matrix& matrix, ID3D11ShaderResourceView* texture)
{
